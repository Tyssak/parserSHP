...
void MultiPointZ::read(std::ifstream& file, std::ofstream& outputFile) {
    // Czytaj bounding box
    ...
    file.read(reinterpret_cast<char*>(&multiPoint.z_min), sizeof(multiPoint.z_min));
    file.read(reinterpret_cast<char*>(&multiPoint.z_max), sizeof(multiPoint.z_max));

    // Czytaj poszczegolne punkty Z
    for (int i = 0; i < multiPoint.num_points; i++) {
        double z_value;
        file.read(reinterpret_cast<char*>(&z_value), sizeof(z_value));
        multiPoint.z_vector.push_back(z_value);
    }
    ...
    for (int i = 0; i < multiPoint.num_points; i++) {
        ...
        outputFile << " Z: " << multiPoint.z_vector.at(i) << " ";
