...
void PolygonZ::read(std::ifstream& file, std::ofstream& outputFile)
{
	PolygonMBox polygon;
	...
	file.read(reinterpret_cast<char*>(&polygon.z_min), sizeof(polygon.z_min));
	file.read(reinterpret_cast<char*>(&polygon.z_max), sizeof(polygon.z_max));
	for (int i = 0; i < polygon.num_points; i++)
	{
		double z_value;
		file.read(reinterpret_cast<char*>(&z_value), sizeof(z_value));
		polygon.z_vector.push_back(z_value);
	}
	...
	for (int i = 0; i < polygon.parts.size(); ++i)
	{
		...
		while (!end_of_part)
		{
			try
			{
				...
				outputFile << "Z: " << polygon.z_vector.at(points_counter - 1) << " ";
